...
ast_composite_t *ir_gen_find_class(object_t *object, const ast_type_t *from_the_type){
    if(from_the_type->elements_length != 1) return NULL;

    ast_t *ast = &object->ast;

    switch(from_the_type->elements[0]->id){
    case AST_ELEM_BASE: {
            const char *target_name = ((ast_elem_base_t*) from_the_type->elements[0])->base;

            for(length_t i = 0; i != ast->composites_length; i++){
                ast_composite_t *composite = &ast->composites[i];

                if(composite->is_class && streq(composite->name, target_name)){
                    return composite;
                }
            }
        }
    ...
    case AST_ELEM_GENERIC_BASE: {
            ast_elem_generic_base_t *generic_base_elem = (ast_elem_generic_base_t*) from_the_type->elements[0];
            ...
            for(length_t i = 0; i != ast->poly_composites_length; i++){
                ...
                if(poly_composite->is_class && streq(poly_composite->name, target_name) && poly_composite->generics_length == generics_count){
