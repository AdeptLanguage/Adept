...
int assemble_type_mappings(compiler_t *compiler, object_t *object){
    ...
    #define ASSEMBLE_BASE_TYPE_MAPPINGS_COUNT 14

    ir_type_t *tmp_type;
    ir_type_map_t *type_map = &module->type_map;
    type_map->mappings_length = ast->structs_length + ASSEMBLE_BASE_TYPE_MAPPINGS_COUNT;
    ...
    for(length_t i = ASSEMBLE_BASE_TYPE_MAPPINGS_COUNT; i != type_map->mappings_length; i++){
        // Create skeletons for struct type maps
        ast_struct_t *structure = &ast->structs[i - ASSEMBLE_BASE_TYPE_MAPPINGS_COUNT];
    ...
    for(length_t i = 0; i != type_map->mappings_length; i++){
        ...
        for(length_t s = 0; s != structure->field_count; s++){
            if(assemble_resolve_type(compiler, object, &structure->field_types[s], &composite->subtypes[s])) return 1;
        }
    }
    ...
    case AST_ELEM_FUNC: {
            ...
            *resolved_type = ir_pool_alloc(&ir_module->pool, sizeof(ir_type_t));
            ...
            for(length_t a = 0; a != function->arity; a++){
                if(assemble_resolve_type(compiler, object, &function->arg_types[a], &extra->arg_types[a])) return 1;
            }

            if(assemble_resolve_type(compiler, object, function->return_type, &extra->return_type)) return 1;
        }
        break;
    default: {
...
bool ast_types_conform(ir_builder_t *builder, ir_value_t **ir_value, ast_type_t *ast_from_type, ast_type_t *ast_to_type, trait_t mode){
    ...
    if(to_type_kind == TYPE_KIND_BOOLEAN && (from_type_kind != TYPE_KIND_NONE || (from_traits & TYPE_TRAIT_BASE_PTR || from_traits & TYPE_TRAIT_POINTER))){
        ...
        if(assemble_resolve_type(builder->compiler, builder->object, ast_to_type, &ir_to_type)) return 1;
