...
int assemble(compiler_t *compiler, object_t *object){
    ...
    if(assemble_type_mappings(compiler, object)) return 1; // Generate type mappings

    if(assemble_globals(compiler, object)) return 1; // Assemble global variables
    if(assemble_functions(compiler, object)) return 1; // Assemble function skeltons
    if(assemble_functions_body(compiler, object)) return 1; // Assemble function bodies
    return 0;
}

int assemble_functions(compiler_t *compiler, object_t *object){
    // NOTE: Only assembles function skeletons
    ...
    for(length_t f = 0; f != ast_funcs_length; f++){
        ...
        if(!(ast_func->traits & AST_FUNC_FOREIGN)){
            module_func->var_map.mappings = malloc(sizeof(ir_var_mapping_t) * 4);
            module_func->var_map.mappings_length = 0;
            module_func->var_map.mappings_capacity = 4;
            ...
            while(module_func->arity != ast_func->arity){
                if(module_func->var_map.mappings_length == module_func->var_map.mappings_capacity){
                    module_func->var_map.mappings_capacity *= 2;
                    ir_var_mapping_t *new_mappings = malloc(sizeof(ir_var_mapping_t) * module_func->var_map.mappings_capacity);
                    memcpy(new_mappings, module_func->var_map.mappings, sizeof(ir_var_mapping_t) * module_func->var_map.mappings_length);
                    free(module_func->var_map.mappings);
                    module_func->var_map.mappings = new_mappings;
                }

                if(assemble_resolve_type(compiler, object, &ast_func->arg_types[module_func->arity], &module_func->argument_types[module_func->arity])) return 1;

                ir_var_mapping_t *mapping = &module_func->var_map.mappings[module_func->var_map.mappings_length++];
                mapping->name = ast_func->arg_names[module_func->arity];
                mapping->var.traits = TRAIT_NONE;
                mapping->var.type = module_func->argument_types[module_func->arity];

                module_func->arity++;
            }
        } else {
            module_func->var_map.mappings = NULL;
            module_func->var_map.mappings_length = 0;
            module_func->var_map.mappings_capacity = 0;
        ...
        if(assemble_resolve_type(compiler, object, &ast_func->return_type, &module_func->return_type)) return 1;
    }
...
int assemble_functions_body(compiler_t *compiler, object_t *object){
    // NOTE: Only assembles function body; assumes skeleton already exists
    ...
    for(length_t f = 0; f != ast_funcs_length; f++){
        ...
        if(assemble_func_statements(compiler, object, &ast_funcs[f], &module_funcs[f])) return 1;
    }
...
int assemble_globals(compiler_t *compiler, object_t *object){
    ...
    for(length_t g = 0; g != ast->globals_length; g++){
        ...
        if(assemble_resolve_type(compiler, object, &ast->globals[g].type, &module->globals[g].type)) return 1;
...
int assemble_globals_init(ir_builder_t *builder){
    ...
    for(length_t g = 0; g != globals_length; g++){
        ...
        if(assemble_expression(builder, ast_global->initial, &value, false, &value_ast_type)) return 1;
